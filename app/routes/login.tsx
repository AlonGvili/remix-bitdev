	...
	return (
		<div className="w-screen h-screen flex flex-col justify-center items-center">
			<div className="items-center flex flex-col max-w-sm w-full p-4">
				...
				<Form action="/auth/google" method="post" className="max-w-sm w-full flex justify-center mb-4">
				...
				<Form action="/auth/github" method="post" className="max-w-sm w-full flex justify-center">
				...
				<Form method="post" action="/auth/form" className="flex flex-col space-y-5 w-full">
					{error && !error.message.includes("Email is required") && !error.message.includes("Password is required") && (
						<p className="text-red-400 text-center font-medium">{error.message}</p>
					)}
					<input type="text" placeholder="Email" name="email" className="py-2 px-8 w-full bg-gray-200 rounded-md" />
					{error && error.message.includes("Email is required") && <p className="text-red-400 text-left font-medium">{error.message}</p>}
					<input type="password" placeholder="Password" name="password" className="py-2 px-8 w-full bg-gray-200 rounded-md" />
					{error && error.message.includes("Password is required") && (
						<p className="text-red-400 text-left font-medium">{error.message}</p>
					)}
					<button
						type="submit"
