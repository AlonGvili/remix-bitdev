...
export let action: ActionFunction = async ({ request }) => {
  ...
  if (action === "delete") {
    if (!(loggedinUser instanceof Error)) {
      let currentUserProfile = await getUser({
        where: {
          id: loggedinUser?.id!,
        },
      });
      let user = await db?.user.update({
        ...
        data: {
          organizationsAdmin: {
            delete: {
              id: currentUserProfile?.organizationsAdmin?.find(
                (o) => o.name === orgName
              )?.id,
            },
          },
          ...
          return (
            <div className="flex w-full border border-gray-200 justify-between items-center p-4 rounded-lg">
              ...
              <div className="space-x-4 flex items-center">
                <h3 className="font-bold text-gray-400 lowercase">
                  {organization.plan}
                </h3>
...
function DeleteOrgButton({ orgName }: { orgName: string }) {
  ...
  return (
    <button
      onClick={() =>
        submit(
          { _action: "delete", orgName: orgName },
          { action: "/settings/organizations", method: "post" }
        )
      }
    >
