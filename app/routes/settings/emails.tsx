...
import { ActionFunction, Form, LoaderFunction, useLoaderData, useLocation, useSubmit } from "remix";
...
export let action: ActionFunction = async ({ request }) => {
	...
	if (action === "delete") {
		if (!(loggedinUser instanceof Error)) {
			...
			let emails = currentUserProfile?.profile?.emails.filter((e) => e !== email);
			let user = await db?.user.update({
				where: {
					id: loggedinUser?.id!
				},
				...
				<div className="space-y-2">
					...
					<div className="flex flex-col">
						...
						{data?.profile?.emails?.map((email: string, index: number) => {
							return (
								<div className="flex justify-between p-4" key={`${email}-${index}`}>
...
function DeleteEmailButton({ email }: { email: string }) {
	...
	return (
		<button onClick={() => submit({ _action: "delete", email: email }, { action: "/settings/emails", method: "post" })}>
...
function MakePrimaryEmailButton({ email }: { email: string }) {
	...
	return (
		<button
			onClick={() => submit({ _action: "makePrimary", email: email }, { action: "/settings/emails", method: "post" })}
