...
import { RiCloseFill, RiTeamFill, RiUser6Fill, RiUserFill } from "react-icons/ri";
import { ActionFunction, Form, Link, LoaderFunction, redirect, useActionData, useLocation, useLoaderData, Links } from "remix";
	...
	return (
		<div className="w-screen h-screen z-50 absolute overflow-x-hidden inset-0 bg-white">
			...
			<div className="flex flex-col max-w-2xl py-24 space-y-4 mx-auto my-auto justify-items-center h-full">
				<Form method="post" className="space-y-8 max-w-2xl pb-12 flex flex-col justify-between">
					<h3 className="text-base font-bold text-gray-300">Step 1 of 3</h3>
					<h1 className="font-bold text-3xl text-gray-600">Upgrade your plan and start 30 days free</h1>
					<RadioGroup value={plan} onChange={setPlan} className="grid grid-cols-2 gap-6">
						<RadioCard value="business">
							<div className="flex flex-col items-center justify-center space-y-4">
								...
								<h1 className="font-semibold text-gray-700 text-sm">For teams building modern apps</h1>
								<ul role="list" className="marker:text-violet-500 marker:text-lg list-disc pl-5 space-y-2 text-slate-400">
									<li className="font-semibold text-sm text-gray-800">Unlimited private scopes</li>
									<li className="font-semibold text-sm text-gray-800">5 members included *</li>
									<li className="font-semibold text-sm text-gray-800">Team management</li>
									<li className="font-semibold text-sm text-gray-800">Autoscaling CI with 3000 minutes</li>
									<li className="font-semibold text-sm text-gray-800">Integrations (GitHub, Slack, WebHooks)</li>
									<li className="font-semibold text-sm text-gray-800">Private component registry</li>
									<li className="font-semibold text-sm text-gray-800">Advanced invoice settings</li>
									<li className="font-semibold text-sm text-gray-800">Premium support</li>
								</ul>
							</div>
						...
						<RadioCard value="pro">
							<div className="flex flex-col items-center justify-center space-y-4">
								...
								<h1 className="font-semibold text-gray-700 text-sm">For developers building small apps</h1>
								<ul role="list" className="marker:text-gray-800 marker:text-lg list-disc pl-5 space-y-2 text-slate-400">
									<li className="font-semibold text-sm text-gray-800">Unlimited private scopes</li>
									<li className="font-semibold text-sm text-gray-800">Add collaborators *</li>
									<li className="font-semibold text-sm text-gray-800">Autoscaling CI with 300 minutes</li>
								</ul>
							</div>
							<div className="w-full space-y-4 text-center">
								<div className="border border-gray-200 w-full" />
					...
					<div className="text-center space-y-4">
						<p className="text-sm font-bold">* $15/month per additional member/collaborator.</p>
						<p className="text-sm font-bold">
							All plans are flexible. You can add and remove members at any time. Your monthly billing will update as you go. First 30
							days free.
						</p>
					</div>
					...
					{plan === "business" && (
						<>
							...
							<RadioGroup value={action} onChange={setAction} className="grid grid-cols-2 gap-4">
								<RadioCard value="transfer" className="flex items-start flex-col p-8 space-y-4">
									<h1 className="text-left font-bold text-base">Transform account</h1>
									<p className="text-sm font-semibold">
										Transform your account to an organization account to keep your current component paths and collaborators.
									</p>
								</RadioCard>
								<RadioCard value="create" className="flex items-start flex-col p-8 space-y-4">
									<h1 className="text-left font-bold text-base">Create Team</h1>
									<p className="text-sm font-semibold">
										Create new organization so you can gradually move your components and add team members
									</p>
								</RadioCard>
							</RadioGroup>
							<Link to={`/${loaderData?.profile?.displayName}/migrate-confirmation`}>
								<button
									type="button"
						...
						<>
							...
							<div className="flex items-center space-x-2">
								{loaderData?.profile?.avatar && (
									<img src={loaderData?.profile?.avatar} className="rounded-full w-12 h-12 object-cover" />
								)}
								<div>
									<p className="text-gray-700 text-sm font-bold">{loaderData?.profile?.name}</p>
									<p className="text-gray-700 text-sm font-bold">Personal account</p>
								</div>
							</div>
							...
							<fieldset className="grid grid-cols-2 gap-4">
								...
								{actionData?.error && actionData.fieldError === "creditCardNumber" && (
									<p className="text-rose-600 text-sm font-bold col-span-full">{actionData.message}</p>
								)}
								<label className="flex flex-col space-y-2">
									<input
										name="creditCardDate"
							...
							<button type="submit" className="bg-violet-500 hover:bg-violet-600 text-white font-bold py-2 px-5 rounded-md max-w-max">
								Submit
							</button>
						</>
					)}
